...
4. The helper script `scripts/query_tcga_api.py` provides compact one-line summaries via its `summarize()` function. Use it when you want deterministic formatting:
   - `python .codex/skills/xena-tcga-gene-query/scripts/query_tcga_api.py diff-expr --gene TP53 --cancer LUAD`
...
7. Include the endpoint, gene/cancer inputs, and any alias resolution shown by `gene_input`, `gene1_input`, or `gene2_input`.
...
```bash
python .codex/skills/xena-tcga-gene-query/scripts/query_tcga_api.py cancers
python .codex/skills/xena-tcga-gene-query/scripts/query_tcga_api.py diff-expr --gene TP53 --cancer LUAD
python .codex/skills/xena-tcga-gene-query/scripts/query_tcga_api.py corr --gene TP53 --gene2 EGFR --cancer LUAD
python .codex/skills/xena-tcga-gene-query/scripts/query_tcga_api.py survival --gene TP53 --cancer LUAD
```
...
If the API is unreachable at both the remote and local URLs, report the errors and give the local setup steps (see Quick Workflow step 1) so the user can install and start the service.
